...
export function CarrierProfileForm({ profile, onSave, onCancel }: { profile: CarrierProfile | null; onSave: (input: Omit<CarrierProfile, "id">) => void; onCancel?: () => void }) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const input = {
      ...
      vehiclePlate: String(values.get("vehiclePlate") ?? "").trim().toUpperCase(),
    };
    if (!input.name) return setError("Укажите имя перевозчика.");
    if (!input.phone) return setError("Укажите номер телефона.");
  ...
  return <section className="profile-setup" aria-labelledby="carrier-profile-title"><div className="profile-setup-card"><div className="profile-setup-head"><span><Icon name="truck" className="size-6"/></span><div><p className="eyebrow">Локальный профиль · без авторизации</p><h1 id="carrier-profile-title">{profile ? "Изменить профиль" : "Профиль перевозчика"}</h1><p>Данные сохранятся только в этом браузере и будут записаны в принятые заказы.</p></div></div><form onSubmit={handleSubmit}><div className="form-grid"><label><span>Имя *</span><input name="name" required autoComplete="name" defaultValue={profile?.name ?? ""} placeholder="Например, Ерлан"/></label><label><span>Телефон *</span><input name="phone" required type="tel" autoComplete="tel" defaultValue={profile?.phone ?? ""} placeholder="+7 777 123 45 67"/></label><label><span>Тип транспорта *</span><select name="vehicleType" required defaultValue={profile?.vehicleType ?? ""}><option value="" disabled>Выберите транспорт</option>{vehicleTypes.map((type) => <option key={type}>{type}</option>)}</select></label><label><span>Госномер *</span><input name="vehiclePlate" required autoComplete="off" defaultValue={profile?.vehiclePlate ?? ""} placeholder="123 ABC 12"/></label></div>{error && <div className="inline-feedback error" role="status"><span>{error}</span></div>}<div className="profile-form-actions">{onCancel && <button type="button" className="secondary-button" onClick={onCancel}>Отмена</button>}<button type="submit" className="primary-button">Сохранить профиль <Icon name="arrow" className="size-4"/></button></div></form></div></section>;
}
