...
import type { RankedRouteMatch, RouteMatch } from "@/types/cargo";
...
export async function acceptAndPersistRouteMatch(match: RankedRouteMatch): Promise<void> {
  const db = requireFirestore();
  ...
  await runTransaction(db, async (transaction) => {
    ...
    await stageOrderAcceptance(transaction, returnOrderRef);
